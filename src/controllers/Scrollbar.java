...
public class Scrollbar extends Controller implements ControllerListener<Scroller> {
	...
	public static class Scroller extends Controller {		
		...
		public void translate(float dx, float dy) {
			if (isHorizontallyOriented()) {
				rect.setCenter(PApplet.constrain(rect.getCenx() + dx, min + rect.getWidth()/2f, max - rect.getWidth()/2f), getCeny());
			}
			else {
				rect.setCenter(getCenx(), PApplet.constrain(rect.getCeny() + dy, min + rect.getHeight()/2f, max - rect.getHeight()/2f));
			}			
		}
